...
private:
	...
	double m_timeLength;	/* ゲームが終了し、フェードアウトが始まった（曲が始まってからの）時刻を保存 */
	...
	//double m_timeCountAbort;	/* 1回目のEscが押された時刻 */
};
...
bool CTyping::idle(double timeCount){	/* 続けるなら（終了するとき以外） true を返す */
	...
	if(m_phase == PHASE_FADEOUT){
		...
		if(time >= m_timeLength + SEC_FADEOUT){	/* 最後の音符が通過した後フェードアウトが終了した */
			phase(PHASE_RESULT);
			return true;
		}
	}
...
void CTyping::phase(int phaseNum){
	...
	if(phaseNum == PHASE_FADEOUT){	/* 終わったのでフェードアウトする */
		m_timeLength = getTime();	/* 終わった時刻を保存 */
...
/* isSaikiは外からの呼び出しではfalse,内からの呼び出しではtrue */
	...
	while(1){
		...
		for(vector<ConvertData>::iterator i = trie->m_data.begin();
			...
			if((*i).match_front(typeBuffer)){
				...
				if(input_1(tmpTypeBuffer, tmpLen, tmpLyricsPos, f_successive, time, isCheckOnly, true)){
				/* 再帰の結果打てることが分かったとき */
...
int CTyping::getBlockStr(vector<LyricsBlock>::iterator itr, char *buf) const{
	int len = 0;
	for(int j=(*itr).nTyped; j<(*itr).lyricsLen; j++){	/* ブロック内の未タイプの文字をbufに格納 */
...
void drawMain(DrawMainInfo &dInfo, CChallenge &challenge,
	...
	SetDrawArea(0, 0, W_WINDOW, H_WINDOW);	/* 描画範囲を元に戻す */
		...
		if(dInfo.rankingPos >= 0){
			/* ランキング表示（rankingPosからRANKING_DRAW_LEN個） */
			(*musicInfoItr).drawRanking(90 + 10, dInfo.rankingPos, RANKING_DRAW_LEN);
		}else{
			(*musicInfoItr).drawComment(90 + 10);
		}
	}
#endif
	...
	if(inputHandle == -1){	/* 検索時以外 */
		DrawStringToHandle(10, 370, "↑/↓: 曲選択,   F: 曲検索, R: ランダム,   F5: リスト再読,",
			GetColor(255, 255, 255), fontHandleDefault);
		
		DrawStringToHandle(10, 390, "←/→: 情報/ランキング表示, Tab: 全画面切り替え,",
			GetColor(255, 255, 255), fontHandleDefault);
