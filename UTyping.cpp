...
void CTyping::load(const char *fumenFileName, const char *rankingFileName){
	...
	fclose(fp);
	if(m_lyricsBlock.empty()){
		//throw "[譜面] データがありません。";
		m_timeLength = 0.0;
	}else{
		m_timeLength = (*(m_lyricsBlock.end() - 1)).timeJust;	/* 譜面終了時刻を記録 */
		/* 【これは、最後に'/'があることを前提としている】 */
	}
...
#if 0
		if(!(*tmpLyricsPosition).isValid){	/* 区切りなど */
			continue;
		}
#endif
