...
void CEffectStr::draw(int X, int Y, double time){
	...
	while(!m_deqCleared.empty() &&
			time - m_deqCleared.front().time >= CEFFECTSTR_SEC){	/* 消えてから十分長い時間がたった */
...
void CTyping::drawGauge(bool isResult){
	...
	for(int i=0; i<GAUGE_COUNT; i++){
		if(i<cnt){
			SetDrawBlendMode(DX_BLENDMODE_ALPHA, 48);	/* 不透明さ3/16 */
			/*
...
*/
	m_effectStr.update(m_typeBuffer, m_typeBufferLen, time);	/* 念のため【本来は不要】 */
